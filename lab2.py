...
def move_handle(build_ele, handle_prop, input_pnt, doc):
    ...
    if handle_prop.handle_id == "BeamHeight":
        ...
        HoleHeight = build_ele.BeamHeight.value - build_ele.TopShHeight.value - RADIUS
...
def beam_height(build_ele, value):
    ...
    if diff < 0:
        ...
        if build_ele.TopShHeight.value > TOPHEIGHT:
            if build_ele.TopShHeight.value - diff < TOPHEIGHT:
                diff -= build_ele.TopShHeight.value - TOPHEIGHT
                build_ele.TopShHeight.value = TOPHEIGHT
            else:
                build_ele.TopShHeight.value -= diff
                diff = 0.0
        if (diff != 0) and (build_ele.BotShUpHeight.value > BOTUPHEIGHT):
            if build_ele.BotShUpHeight.value - diff < BOTUPHEIGHT:
                diff -= build_ele.BotShUpHeight.value - BOTUPHEIGHT
                build_ele.BotShUpHeight.value = BOTUPHEIGHT
            else:
                build_ele.BotShUpHeight.value -= diff
                diff = 0.0
        if (diff != 0) and (build_ele.BotShLowHeight.value > BOTLOWHEIGHT):
            if build_ele.BotShLowHeight.value - diff < BOTLOWHEIGHT:
                diff -= build_ele.BotShLowHeight.value - BOTLOWHEIGHT
                build_ele.BotShLowHeight.value = BOTLOWHEIGHT
            else:
                build_ele.BotShLowHeight.value -= diff
                diff = 0.0
        if (diff != 0) and (build_ele.RibHeight.value > RIBHEIGHT):
            if build_ele.RibHeight.value - diff < RIBHEIGHT:
                diff -= build_ele.RibHeight.value - RIBHEIGHT
                build_ele.RibHeight.value = RIBHEIGHT
            else:
                build_ele.RibHeight.value -= diff
                diff = 0.0
    else:
        build_ele.RibHeight.value += diff
    if value - build_ele.TopShHeight.value - RADIUS < build_ele.HoleHeight.value:
        build_ele.HoleHeight.value = value - build_ele.TopShHeight.value - RADIUS
    return build_ele
...
def modify_element_property(build_ele, name, value):
    if name == "BeamHeight":
        ...
        HoleHeight = value + build_ele.BotShLowHeight.value + RADIUS
        ...
        HoleHeight = build_ele.BotShUpHeight.value + value + RADIUS
        if  HoleHeight > build_ele.HoleHeight.value:
            build_ele.HoleHeight.value = HoleHeight
    elif name == "HoleHeight":
        HoleHeightFromTop = build_ele.BeamHeight.value - build_ele.TopShHeight.value - RADIUS
        HoleHeightFromBot = build_ele.BotShLowHeight.value + build_ele.BotShUpHeight.value + RADIUS
        ...
        if value >= HoleDepth:
            build_ele.HoleDepth.value = HoleDepth - RADIUS
